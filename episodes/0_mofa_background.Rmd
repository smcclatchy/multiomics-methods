...
# Github will not load MotrpacRatTraining6mo because it's too big. Instead,
# define MotrpacRatTraining6mo functions to get the website to build on Github.
# the following are adapted from 
# https://github.com/MoTrPAC/MotrpacRatTraining6mo/blob/bea2ab1120c0bc0a4f4aa9602e93ecf5685e54fb/R/utils.R#L44C1-L50C2
# and 
# https://github.com/MoTrPAC/MotrpacRatTraining6mo/blob/bea2ab1120c0bc0a4f4aa9602e93ecf5685e54fb/R/fetch_prep_data.R#L403
...
tissue_abbrev <- load(file.path(data_dir, 'tissue_abbrev.Rdata'))
assay_abbrev <- load(file.path(data_dir, 'assay_abbrev.Rdata'))
metab_norm_data_flat <- load(file.path(data_dir, 'metab_norm_data_flat.Rdata'))
immuno_norm_data_flat <- load(file.path(data_dir, 'immuno_norm_data_flat.Rdata'))
outliers <- load(file.path(data_dir, 'outliers.Rdata'))
available_data <- load(file.path(data_dir, 'available_data.Rdata'))
...
load_sample_data = function(tissue, 
  ...
  # check inputs 
  if(!tissue %in% tissue_abbrev){
    stop(sprintf("'tissue' must be one of TISSUE_ABBREV: \n %s", 
                 paste(tissue_abbrev, collapse=", ")))
  }
  if(!assay %in% assay_abbrev){
    stop(sprintf("'assay' must be one of ASSAY_ABBREV: \n %s", 
                 paste(assay_abbrev, collapse=", ")))
  }
  ...
  if(exclude_outliers){
    outliers = data.table::data.table(outliers)
    .assay = assay
    .tissue = tissue
